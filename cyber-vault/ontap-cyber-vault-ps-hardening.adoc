...
ONTAP proporciona un conjunto de controles que permiten fortalecer el almacenamiento de ONTAP .  Utilice ellink:https://docs.netapp.com/us-en/ontap/ontap-security-hardening/security-hardening-overview.html["Orientación y configuración de ONTAP"^] para ayudar a la organización a cumplir los objetivos de seguridad prescritos para la confidencialidad, integridad y disponibilidad del sistema de información.
...
. Asegúrese de que las políticas de enrutamiento del firewall estén configuradas correctamente y que todas las reglas se auditen y actualicen periódicamente según sea necesario.
...
. Protege la administración con la verificación multiadministrador (MAV) además de la autenticación multifactor (MFA), mejorando la seguridad para el acceso administrativo a la VM de almacenamiento de datos.
. Habilitar el cifrado para datos estándar "en tránsito" entre clústeres.
. Asegure SSH con un cifrado fuerte y aplique contraseñas seguras.
. Habilitar FIPS global.
. Telnet y Remote Shell (RSH) deben estar deshabilitados.
. Bloquear la cuenta de administrador predeterminada.
. Deshabilite los LIF de datos y proteja los puntos de acceso remoto.
. Deshabilite y elimine protocolos y servicios no utilizados o extraños.
. Cifrar el tráfico de red.
. Utilice el principio del mínimo privilegio al configurar roles administrativos y de superusuario.
. Restrinja HTTPS y SSH desde una dirección IP específica usando la opción de IP permitida.
. Poner en pausa y reanudar la replicación según el programa de transferencia.


Los puntos 1 a 4 requieren intervención manual, como designar una red aislada, segregar el espacio IP, etc., y deben realizarse de antemano.  La información detallada para configurar el endurecimiento se puede encontrar en ellink:https://docs.netapp.com/us-en/ontap/ontap-security-hardening/security-hardening-overview.html["Guía de refuerzo de seguridad de ONTAP"^] .  El resto se puede automatizar fácilmente para facilitar la implementación y el monitoreo.  El objetivo de este enfoque orquestado es proporcionar un mecanismo para automatizar los pasos de fortalecimiento para proteger el controlador de la bóveda en el futuro.  El período durante el cual el espacio de aire de la bóveda cibernética permanece abierto es el más breve posible.  SnapVault aprovecha la tecnología incremental permanente, que solo moverá los cambios desde la última actualización a la bóveda cibernética, minimizando así la cantidad de tiempo que la bóveda cibernética debe permanecer abierta.  Para optimizar aún más el flujo de trabajo, la apertura de la bóveda cibernética se coordina con el cronograma de replicación para garantizar la ventana de conexión más pequeña.

Aquí hay un ejemplo de código de PowerShell para fortalecer un controlador ONTAP .
