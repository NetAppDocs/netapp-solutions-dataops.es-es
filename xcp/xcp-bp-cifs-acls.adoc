...
summary: Esta sección cubre el procedimiento paso a paso para migrar datos CIFS con información de seguridad desde un sistema ONTAP de origen a un sistema ONTAP de destino. 
---
= Migración de datos CIFS con ACL desde un servidor de almacenamiento de origen a ONTAP
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Esta sección cubre el procedimiento paso a paso para migrar datos CIFS con información de seguridad desde un sistema ONTAP de origen a un sistema ONTAP de destino.
...
NOTE: Si no hay ningún agregado de datos, cree uno nuevo utilizando el `storage aggr create` dominio.
...
....
. Cree un LIF de datos para atender solicitudes de clientes SMB.
+
....
...
2 entries were displayed.
....
. Monte el volumen de datos de destino en el espacio de nombres SVM.
+
....
...
....
. Inicie el servicio CIF en el SVM de destino.
+
....
...
....
. Verifique que la política de exportación predeterminada se aplique al SVM de destino.
+
....
...
....
. Modificar las reglas de la política de exportación para permitir el acceso a clientes CIF.
+
....
...
....
+
Verificar que las reglas de la política se modifiquen.
...
2 entries were displayed.
....
. Conéctese al sistema cliente Windows donde está instalado XCP.  Busque la ruta de instalación de XCP.
+
....
C:\WRSHDNT>dir c:\netapp\xcp
dir c:\netapp\xcp
...
....
. Consulte las exportaciones SMB del nodo de origen ejecutando el comando `xcp show` comando en el sistema host del cliente Windows XCP.
+
....
...
....
. Ejecutar el `help` Comando para copiar.
+
....
...
....
. En el sistema ONTAP de destino, obtenga la lista de nombres de usuarios y grupos locales que necesita proporcionar como valores para `fallback-user` y `fallback-group` ruta de argumentos.
+
....
...
5 entries were displayed
....
. Para migrar los datos CIF con ACL desde el origen al destino, ejecute el comando `xcp copy` Comando con el `-acl` y `–fallback-user/group` opciones.
+
Para el `fallback-user/group` opciones, especifique cualquier usuario o grupo que se pueda encontrar en Active Directory o usuario/grupo local en el sistema de destino.
...
C:\WRSHDNT>
....
. Si `xcp copy` da como resultado el mensaje de error `ERROR failed to obtain fallback security principal` , agregue el cuadro de destino en el archivo de hosts(`C:\Windows\System32\drivers\etc\hosts` ).
+
Utilice el siguiente formato para la entrada del cuadro de destino de almacenamiento de NetApp .
...
....
. Si sigue recibiendo el mensaje de error `ERROR failed to obtain fallback security principal` Después de agregar la entrada del cuadro de destino en los archivos de hosts, el usuario/grupo no existe en el sistema de destino.
+
....
...
C:\WRSHDNT>
....
. Usar `xcp copy` para migrar datos CIF con ACL (con o sin la carpeta raíz).
+
